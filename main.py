...
class DigitSimulation:
    ...
    def solve_MPCC_and_QP(self):
        q, dq, _ = self.output()
        
        self.s = np.min([(self.data.time-self.Tk_1)/self.MPC.Tst,1.0])
        
        x0, _ = self.PWQP._extractLIPStatefromFull()
        PStFoot = self.T_StFoot[0:3,3].reshape((3,1))
        
        isRightStance = self.state == StanceState.RIGHT_STANCE.name
        
        ##### Solve MPCC        
        if self.data.time==0.0 or self.data.time-self.t_MPC_lastrun>0.03: # 100Hz
            try:
                x_end = self.MPC.predictTouchDownLIPState(x0, self.s, self.MPC.Tst, PStFoot[0:2])
                
                u_MPC, self.des_thSwFk, _, _, _, _ = self.MPC.Solve(self.data.time, x_end.squeeze(), self.thSwFk_1, not isRightStance)
                
                self.p_fst = x_end[0:2].reshape((2,1)) + u_MPC.reshape((2,1))
                self.t_MPC_lastrun = self.data.time
            except:
                print('MPC failed')

        des_X_LIP = self.MPC._CLSolution_abs(x0, self.data.time-self.t_MPC_lastrun, self.PSwFk_1)
        
        ##### Solve QP
        try:
            self.ctrl = self.PWQP.WalkingQP(q,dq,isRightStance,self.s,self.PSwFk_1,np.array([self.p_fst[0,0],self.p_fst[1,0],0]),self.thSwFk_1,self.MPC.H,self.MPC.Tst,des_X_LIP,self.des_thSwFk)
        except:
            print('QP failed')
